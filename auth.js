...
(function () {
  // Google's official multicolor "G" mark (unmodified brand icon), used on
  // the "Continue with Google" buttons per Google's sign-in button guidelines.
  const GOOGLE_ICON_SVG = `<svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
  ...
  const MODAL_HTML = `
    <div class="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-modal-title-login">
      ...
      <form class="auth-form" id="auth-form-login" data-mode="login" novalidate>
        ...
        <button type="button" class="auth-google-btn" data-google-signin>
          ${GOOGLE_ICON_SVG}
  ...
  // Both forms' "Continue with Google" buttons share this handler.
  // redirectTo sends the person back to the exact page they started from
  // (Supabase's OAuth flow otherwise lands on a blank Supabase-hosted page)
  // — the auth listener below then picks up the returned session the same
  // way it does for email/password logins.
  async function signInWithGoogle(errorEl) {
    errorEl.hidden = true;
    errorEl.textContent = '';
    try {
      ...
      if (error) showError(errorEl, friendlyAuthError(error));
    } catch (err) {
      showError(errorEl, "Couldn't connect. Check your connection and try again.");
    }
  }

  overlay.querySelectorAll('[data-google-signin]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const form = btn.closest('.auth-form');
      const errorEl = form.querySelector('.auth-form-error');
      signInWithGoogle(errorEl);
    });
  });
